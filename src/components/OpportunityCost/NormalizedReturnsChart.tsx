...
export default function NormalizedReturnsChart({ series, periods, adminStartUsIndex, usIndex }: Props) {
  ...
  return (
    <section className="oc-chart-panel" aria-label="Cumulative US index return by administration chart">
      <div className="oc-chart-header">
        <h4 title={`Cumulative ${usIndexLabel} return by administration`}>
          Cumulative US index return by administration ({usIndexLabel})
        </h4>
        ...
        <div className="oc-hover-panel oc-hover-panel-main" aria-live="polite">
          <div className="oc-hover-row">
            ...
            <span
              ...
              title={legend ? `Trading day ${legend.day}` : 'Hover chart to inspect cumulative return'}
            >
              {legend ? `D${legend.day}` : 'Hover chart to inspect cumulative return'}
            </span>
          </div>
          {ADMIN_KEYS.map((key) => (
      ...
      <ul className="oc-chart-meta-list" aria-label="Cumulative return chart notes">
        <li>Raw US return only: no peer subtraction in this view.</li>
        <li>Each line starts at 0% on the first available trading day in that administration window.</li>
        <li>Every recent administration window to date shows positive cumulative market returns.</li>
        <li>
